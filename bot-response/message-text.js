...
const msgText = {
  newConfirm: (rotation) => {
    return ':sparkles: The *' + rotation + '* rotation has been created. You can now assign someone to be on-call for this rotation or add a staff list. Use `@uop help` to learn more.';
  },
  newError: (rotation) => {
    return ':mag_right: The *' + rotation + '* rotation already exists. You can assign someone to be on-call with `@uop "' + rotation + '" assign [@user]`.';
  },
  ...
  descError: (rotation) => {
    return ':shrug: I couldn\'t update the description for "' + rotation + '" because it does not exist. When you create a new rotation, you can include the description: `@uop new "' + rotation + '" [description]`.';
  },
  staffConfirm: (rotation) => {
    return ':busts_in_silhouette: The *' + rotation + '* rotation staff list has been saved! You can now use `@uop "' + rotation + '" assign next` to rotate assignments.\nWhen using `next`, if the currently on-call person is not in the staff list, the assignment will default to the _first person_ in the rotation.\n_(Note: I remove duplicates. If you want someone to do additional shifts, you\'ll need to do a username assignment.)_';
  },
  staffEmpty: () => {
  ...
  staffError: (rotation) => {
    return ':shrug: I couldn\'t save staff for "' + rotation + '" because it does not exist. To create this rotation, first tell me `@uop new "' + rotation + '" [description]`, _then_ set up staffing.';
  },
  ...
  aboutError: (rotation) => {
    return ':shrug: I couldn\'t get any info about "' + rotation + '" because it does not exist. To create it, use `@uop new "' + rotation + '" [description]`.';
  },
  ...
  assignNextError: (rotation) => {
    return ':shrug: I couldn\'t assign the next person in the *' + rotation + '* rotation because there is nobody in the rotation staff list.\nTo set staff, use `@uop "' + rotation + '" staff [@user1 @user2 @user3]`.\nThen you can use `@uop "' + rotation + '" assign next [optional handoff message]` to rotate to the next user on staff.\nIf you don\'t want to use a staff list, assign someone directly by username: `@uop "' + rotation +'" assign [@user] [optional handoff message]`.';
  },
  assignError: (rotation) => {
    return ':shrug: I couldn\'t complete this assignment because "' + rotation + '" does not exist. To create it, use `@uop new "' + rotation + '" [description]`.';
  },
  listReport: (list) => {
    let msgStr = '';
  ...
  listEmpty: () => {
    return ':clipboard: There are no rotations saved right now. To create one, tell me `@uop new "[rotation-name]" [description]`.';
  },
  whoReport: (usermention, rotation) => {
    return ':bust_in_silhouette: `' + usermention + '` is on duty for the *' + rotation + '* rotation. To notify them directly, use `@uop "' + rotation + '" [message]`.';
  },
  nobodyAssigned: (rotation) => {
    return ':ghost: Nobody is currently assigned to the *' + rotation + '* rotation. To assign someone, use `@uop "' + rotation + '" assign [@user] [optional handoff message]` or assign the next person in the rotation staff list with `@uop "' + rotation + '" assign next [optional handoff message]`.';
  },
  whoError: (rotation) => {
    return ':shrug: I couldn\'t find any on-call user because "' + rotation + '" does not exist. To create it, use `@uop new "' + rotation + '" [description]`. You can then assign someone using `@uop "' + rotation + '" assign [@user] [optional handoff message]`.';
  },
  ...
  unassignError: (rotation) => {
    return ':shrug: I couldn\'t clear this assignment because "' + rotation + '" does not exist. To create it, use `@uop new "' + rotation + '" [description]`.';
  },
  ...
  msgError: (rotation) => {
    return ':shrug: I couldn\'t tell anyone about the message because "' + rotation + '" does not exist. To create it, use `@uop new "' + rotation + '" [description]`.';
  },
  didntUnderstand: (ec, msgText) => {
    return ":thinking_face: I'm sorry, I didn't understand that. To see my full capabilities and learn how to format commands, type `@uop help`.\nUndecipherable message text: `" + msgText + "`\nResponse log:\n```" + ec + "```";
  },
  error: (err) => {
